...
[![事件管理器示意图](http://remcarpediem.net/images/19_85/image1.png)](http://remcarpediem.net/images/19_85/image1.png)
...
Redis基于Reactor模式开发了自己的网络事件处理器，也就是文件事件处理器。文件事件处理器使用**IO多路复用技术，同时监听多个套接字，并为套接字关联不同的事件处理函数**。当套接字的可读或者可写事件触发时，就会调用相应的事件处理函数。
...
[![示意图](http://remcarpediem.net/images/19_85/image2.png)](http://remcarpediem.net/images/19_85/image2.png)

如下图所示，文件事件处理器有四个组成部分，它们分别是套接字、I/O多路复用程序、文件事件分派器以及事件处理器。

[![示意图](http://remcarpediem.net/images/19_85/image3.png)](http://remcarpediem.net/images/19_85/image3.png)
...
[![一次请求的过程示意图](http://remcarpediem.net/images/19_85/image4.png)](http://remcarpediem.net/images/19_85/image4.png)一次请求的过程示意图
...
`aeFileEvent`是文件事件结构，对于每一个具体的事件，都有**读处理函数**和**写处理函数**等。Redis 调用`aeCreateFileEvent`函数针对不同的套接字的读写事件注册对应的文件事件。
...
[![aeApiPoll示意图](http://remcarpediem.net/images/19_85/image5.png)](http://remcarpediem.net/images/19_85/image5.png)aeApiPoll示意图
