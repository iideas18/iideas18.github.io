...
- Table of Contents
  - [Spark基础知识](https://github.com/linbojin/spark-notes/blob/master/rdd-abstraction.md#spark基础知识)
  - RDD抽象模型
    - [设计动机](https://github.com/linbojin/spark-notes/blob/master/rdd-abstraction.md#设计动机)
    - [定义和特性](https://github.com/linbojin/spark-notes/blob/master/rdd-abstraction.md#定义和特性)
    - [Lineage和Dependencies](https://github.com/linbojin/spark-notes/blob/master/rdd-abstraction.md#lineage和dependencies)
  - Spark的RDD源码阅读
    - [RDD编程接口](https://github.com/linbojin/spark-notes/blob/master/rdd-abstraction.md#rdd编程接口)
    - [RDD函数](https://github.com/linbojin/spark-notes/blob/master/rdd-abstraction.md#rdd函数)
    - [RDD隐式转换](https://github.com/linbojin/spark-notes/blob/master/rdd-abstraction.md#rdd隐式转换)
  - 编写RDD API函数
    - [难点和突破口](https://github.com/linbojin/spark-notes/blob/master/rdd-abstraction.md#难点和突破口)
    - [开发和测试](https://github.com/linbojin/spark-notes/blob/master/rdd-abstraction.md#开发和测试)
    - [使用RDD隐式转换](https://github.com/linbojin/spark-notes/blob/master/rdd-abstraction.md#使用rdd隐式转换)
...
[![image-20200604135045369](C:\Users\zhouy1\AppData\Roaming\Typora\typora-user-images\image-20200604135045369.png)](https://github.com/linbojin/spark-notes/blob/master/media/02-schedulingProcess.jpg)
...
![image-20200604135259233](C:\Users\zhouy1\AppData\Roaming\Typora\typora-user-images\image-20200604135259233.png)
...
### 2.3 Lineage和Dependencies

Spark会记录用于产生某个RDD的所有transformations，称为**Lineage**，更加直白地理解就是一个RDD知道它是如果从它的父辈RDD们转换而来的，下图是一个RDD Lineage Graph，它是一种DAG Graph： 

![image-20200604135412287](C:\Users\zhouy1\AppData\Roaming\Typora\typora-user-images\image-20200604135412287.png)

> Lineage是Spark高效容错的法宝，因为有了Lineage信息，RDD可以重新单独计算某个丢失的partition，而不用计算整个RDD。

Lineage基于RDD的Dependencies，所以Lineage Graph也可以说成**Dependency Graph**。RDD的Dependencies可以分为两类：窄依赖（**Narrow Dependencies**）和宽依赖（**Wide Dependencies**）。下图就是Dependency Graphs, 每个大方框是一个RDD，蓝色的小矩阵是对应的Partitions：

![image-20200604135437696](C:\Users\zhouy1\AppData\Roaming\Typora\typora-user-images\image-20200604135437696.png)
...
有了前面的理论准备，现在可以开始阅读Spark的RDD模型源码实现了。开始之前，请确保你已经参考我的第一篇笔记[搭建Spark源码研读和代码调试的开发环境](https://github.com/linbojin/spark-notes/blob/master/ide-setup.md)，用Intellij IDEA打开了Spark项目，并且编译通过，然后
...
![image-20200604135603267](C:\Users\zhouy1\AppData\Roaming\Typora\typora-user-images\image-20200604135603267.png)
...
- 用于表示RDD的接口函数
  ![image-20200604135704501](C:\Users\zhouy1\AppData\Roaming\Typora\typora-user-images\image-20200604135704501.png)

- Transformations（返回新的RDD）

- Actions (启动一个job来计算结果) 

  ![image-20200604135740497](C:\Users\zhouy1\AppData\Roaming\Typora\typora-user-images\image-20200604135740497.png)

- 这里推荐一个非常棒的文档(文件比较大，可能需要pc端打开)，[Visual Spark Transformations and Actions](https://github.com/linbojin/spark-notes/blob/master/awesome-slides/2015-Visual-Spark-API-Databricks.pdf)，用图很形象地介绍RDD APIs.

- 其他内部使用的方法和属性

现在可以先阅读Transformations函数源码，其他的会在后面的笔记里面涉及，因为Actions都会调用`sc.runJon()`来运行job，深入理解Spark Scheduling之后再阅读会比较容易理解。 这里以**flatMap**为例：
...
`withScope{body}`是为了确保运行body代码块产生的所有RDDs都在同一个scope里面。

`val cleanF = sc.clean(f)`是为了确保这个f函数（专业称为closure）是可以被序列化的，因为它需要被序列化后发送到不同的node上。想要了解他们的实现细节可以用IDE代码跳转功能：
...
iter：Iterator[T]是一个Partition上的元素迭代器，用来遍历RDD[T]的第pid个partition上的所有元素。`firstParent[T].iterator(split, context)`就是返回parentRDD的对应partition的迭代器iter：Iterator[T]： 如果已经保存了就直接读取，否则重新计算（可以跳转看它的实现）。有了这个迭代器iter之后，然后用`iter.flatMap(cleanF)`来产生新的迭代器，返回类型是Iterator[U]，这个就是最终返回的RDD: RDD[U]的partition的迭代器啦。`iter.flatMap(cleanF)`只是纯粹的scala flatMap计算，在不同的node上运行。 这部分内容，可能比较绕，只有看着源码不断的跳转，才能比较好的理解。高度总结一下，Transformations函数的实现，就是对RDD上每一个partition的iterator进行相应的操作。

### 3.3 RDD隐式转换

在RDD.scala文件的最后有一个object RDD，这个很有意思，也体现了scala implicit的强大之处: 

![image-20200604135932903](C:\Users\zhouy1\AppData\Roaming\Typora\typora-user-images\image-20200604135932903.png)
...
![image-20200604140006116](C:\Users\zhouy1\AppData\Roaming\Typora\typora-user-images\image-20200604140006116.png)
...
![image-20200604140123577](C:\Users\zhouy1\AppData\Roaming\Typora\typora-user-images\image-20200604140123577.png)
...
![image-20200604140151225](C:\Users\zhouy1\AppData\Roaming\Typora\typora-user-images\image-20200604140151225.png)
...
![image-20200604140222345](C:\Users\zhouy1\AppData\Roaming\Typora\typora-user-images\image-20200604140222345.png)
...
![image-20200604140244593](C:\Users\zhouy1\AppData\Roaming\Typora\typora-user-images\image-20200604140244593.png)
