...
在Linux内核中，Socket的实现分为三层，第一层是 `GLIBC接口层`，第二层是 `BSD接口层`，第三层是 `具体的协议层`（如Unix sokcet或者INET socket）。如下图所示： ![Image](640.png)
...
通过下面一幅图来展示 `Socket族系统调用` 的原理： ![Image](640-1648278162793.png)
