...
Original songsong001 [Linux内核那些事](javascript:void(0);) *2021-08-10 09:00*

## 1. namespace介绍

`namespace（命名空间）` 是Linux提供的一种内核级别环境隔离的方法，很多编程语言也有 namespace 这样的功能，例如C++，Java等，编程语言的 **namespace 是为了解决项目中能够在不同的命名空间里使用相同的函数名或者类名**。而Linux的 namespace 也是为了**实现资源能够在不同的命名空间里有相同的名称**，譬如在 `A命名空间` 有个pid为1的进程，而在 `B命名空间` 中也可以有一个pid为1的进程。
